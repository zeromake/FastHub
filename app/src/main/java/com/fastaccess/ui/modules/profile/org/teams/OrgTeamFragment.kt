 ...
 */
class OrgTeamFragment : BaseFragment<OrgTeamMvp.View, OrgTeamPresenter>(), OrgTeamMvp.View {
    @JvmField
    ...
    override fun onNotifyAdapter(items: List<TeamsModel>?, page: Int) {
        hideProgress()
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = TeamsAdapter(presenter!!.teams)
        adapter!!.listener = presenter
        recycler!!.adapter = adapter
        recycler!!.addOnScrollListener(loadMore)
        recycler!!.addKeyLineDivider()
        if (presenter!!.teams.isEmpty() && !presenter!!.isApiCalled) {
            onRefresh()
        }
        fastScroller!!.attachRecyclerView(recycler!!)
    }

    override fun providePresenter(): OrgTeamPresenter {
        return OrgTeamPresenter()
    }
    ...
    companion object {
        fun newInstance(username: String): OrgTeamFragment {
            val view = OrgTeamFragment()
