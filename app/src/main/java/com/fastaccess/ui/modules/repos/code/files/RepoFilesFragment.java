...
public class RepoFilesFragment extends BaseFragment<RepoFilesMvp.View, RepoFilesPresenter> implements RepoFilesMvp.View {
    ...
    @Override
    public void onNotifyAdapter(@NonNull List<RepoFile> list) {
    ...
    public void onItemClicked(@NonNull RepoFile model) {
        ...
        if (model.getType() == FilesType.dir) {
            ...
            if (model.getSize() == 0 && InputHelper.isEmpty(model.getDownloadUrl()) && !InputHelper.isEmpty(model.getGitUrl())) {
                RepoFilesActivity.startActivity(requireContext(), model.getGitUrl().replace("trees/", ""), isEnterprise());
            } else {
            ...
            switch (item1.getItemId()) {
                ...
                case R.id.download:
                    if (ActivityHelper.checkAndRequestReadWritePermission(requireActivity())) {
                        RestProvider.downloadFile(requireContext(), item.getDownloadUrl());
                    }
                    break;
                case R.id.copy:
                ...
                case R.id.editFile:
                    if (PrefGetter.isProEnabled() || PrefGetter.isAllFeaturesUnlocked()) {
                        ...
                        PremiumActivity.Companion.startActivity(requireContext());
                    }
                    break;
    ...
    @Override
    public void onSetData(@NonNull String login, @NonNull String repoId, @NonNull String path,
    ...
    public void onDelete(@NonNull String message, int position) {
        getPresenter().onDeleteFile(message, Objects.requireNonNull(adapter.getItem(position)), getParent() != null ? getParent().getRef() : "master");
    }
